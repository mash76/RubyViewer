...
osRunOut = function(command , out_html_id , action , cb){
  ...
  exec(command,execOption, (error, stdout, stderr) => {
    ...
    $('#' + out_html_id).append(sRed(escapeHTML(command)) + " " + sGray(ret_ary.length) )
                        .append( '<pre class="code">' + ret_ary.join('\n') + '</pre>' )
                        .append( sRed(stderr.replace(/\n/g,'<br/>')))

    if (typeof cb == "function") cb()
  });
}

osOutText = function(os_command,id_tag,ret_ary){
  ...
  if (id_tag){
      $('#' + id_tag).html(sRed(escapeHTML(os_command)) + " " + sGray(ret_ary.length) + '<hr/>' + ret_out_str )
  }
}
...
path2pjname =function(full_path){ 
  console.log(full_path);
  ret = full_path.replace(/\/Dockerfile/,'').replace(/.*\//,'')
  console.log(ret)
  return ret 
  } // 後ろの.gitと 直前のスラッシュまでを除去
path2dir =function(full_path){ return full_path.replace(/\/Dockerfile/i,'') }
